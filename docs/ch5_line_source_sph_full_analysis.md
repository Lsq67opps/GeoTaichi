...
- 状态方程（弱可压 Tait，式(4.105)，\(\gamma=7\)，\(c_0=10\sqrt{gH}\) 控制密度涨落 \(\lesssim1\%\)，原式见《论文翻译.md》式(4.105)）  
  \[
...
5. 后处理：以 \(\alpha_s=0.1\alpha_s^m\) 定义云团边界（\(\alpha_s^m\) 为该时刻云团内最大泥沙体积分数），提取宽度 \(B\)、下沉距离 \(Z\)、无量纲速度 \(w_c/\omega_s\)，并可用 \(L_0=\sqrt{q_0}, u_0=\sqrt{gL_0}\) 做归一化。  
